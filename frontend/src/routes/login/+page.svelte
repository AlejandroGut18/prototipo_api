  ...
  async function handleLogin() {
    try {
      const response = await fetch("http://localhost:8000/api/usuarios/", {
        ...
        body: JSON.stringify({
          correo: email,  // Cambia a "username" si Django espera eso
          password: password,
        }),
      });
      ...
      if (response.ok) {
        ...
        window.location.href = "/adminpanel/";  // Redirige a la página de administrador
      } else {
        error = data.error;  // Muestra el mensaje de error de Django
      }
    } catch (err) {
  ...
  <div class="login-form">
    ...
    <div class="form-group">
      <form on:submit|preventDefault={handleLogin}>
        ...
        <input type="email" id="email" bind:value={email} placeholder="Ingresa tu correo" required /><br><br>
        
        <label for="password">Contraseña</label>
        <input type="password" id="password" bind:value={password} placeholder="Ingresa tu contraseña" required/><br><br>

        <!--<button class="login-button" type="submit">Iniciar Sesión</button>-->
        <button class="login-button"><a href="/adminpanel"><b>Iniciar Sesión</b></a></button>
